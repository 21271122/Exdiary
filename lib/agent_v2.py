...
TOOL_ANALYZE = {
    ...
    "function": {
        "name": "analyze",
        "description": (
            "跨实验分析。仅在 record 线程中可用（一键式轻量分析），"
            "或在无线程时启动 analyze 线程。在 analyze 线程中不可用——"
            "analyze 线程中 Agent 通过 search_experiments + load_reference + 自身推理进行深入讨论。"
        ),
        "parameters": {
            "type": "object",
            "properties": {
                ...
                "refs": {"type": "array", "items": {"type": "string"},
                         "description": "参与分析的实验。空数组表示按 query 自动筛选"},
            },
            "required": ["query"],
        },
    },
...
- start_record_thread: 用户要记录新实验时调用，开启实验记录线程。
- modify_experiment: 修改已存在实验的字段。需先 load_reference 获取当前值。
- manage_collection: 置顶/取消置顶/收藏/取消收藏实验。最多置顶3个。
- query_experiment: 查询实验参数（如"003的退火温度是多少"）。
- analyze: 跨实验一键分析（仅在 record 线程或无线程时可用）。
...
class ToolExecutor:
    ...
    def _analyze(self, args: dict, loop: "AgentLoop") -> dict:
        # analyze 线程内不可用
        if loop.thread_id and loop._pending_thread_start is None:
            # 检查是否在 analyze 线程内（通过检查 history 中的线程标记）
            for m in loop.history:
                if f"thread_begin id={loop.thread_id} type=analyze" in (m.get("content") or ""):
                    return {"error": "not_in_analyze_thread",
                            "message": "analyze 工具在 analyze 线程中不可用。请使用 search_experiments + load_reference + 自身推理进行深入讨论。"}
        query = args["query"]
        refs = args.get("refs", [])

        # 设置线程开始 flag（如果不在 record 线程内），并立即注入标记
        if loop.thread_id is None and loop._pending_thread_start is None:
            loop._pending_thread_start = "analyze"
            loop._maybe_inject_thread_start()

        # 收集实验摘要
        try:
            summary = self.store.summarize_all(exp_ids=refs if refs else None)
            ...
            return {"error": "analyze_failed", "message": str(e)[:300]}
...
class AgentLoop:
    ...
    def run(self, user_message: str = "") -> dict:
        ...
        while True:
            ...
            # 构建 LLM 消息：有摘要则用摘要+最近消息，无摘要则完整 history
            messages = [
                {"role": "system", "content": SYSTEM_PROMPT},
                {"role": "system", "content": self._build_thread_status()},
            ]
    ...
    def _build_thread_guidance(self, thread_type: str) -> dict:
        ...
        if thread_type == "record":
            ...
            return {"role": "system",
                    "content": "你正在进行跨实验分析。深入讨论，多轮对话是预期的。使用 search_experiments 定位相关实验，load_reference 加载数据，然后基于数据推理。目标：输出分析报告。"}
